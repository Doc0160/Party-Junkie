...
function make_num_from_xml_filename($t){
    return explode("-", $t)[0];
}
...
for ($a = $nb -1; $a > 1; $a--) {
	...
	<html name="html">
	<head>
		<title>PJ-'.make_num_from_xml_filename($name[$a])."-".
            make_title_from_xml_filename($name[$a]).'</title>
    ...
    while ($i < $nb) {
		$temp = explode("-", $name[$i]);
	    $page .= '<li>'.
                 '<a href="'.make_num_from_xml_filename($name[$i]).'">'.
